...
pipeline {
    ...
    stages {
        ...
        stage ("Checkout") {
            steps {
                timestamps {
                    checkout(changelog: false, poll: false, scm: [$class: "GitSCM", branches: [[name: "*/master"]], doGenerateSubmoduleConfigurations: false, extensions: [[$class: "CloneOption", depth: 0, noTags: true, reference: "", shallow: true]], submoduleCfg: [], userRemoteConfigs: [[url: "${env.CLONE_BASE}/properties-maven-plugin.git"]]])
                }
            }
        }
        stage ("Build") {
            ...
            post {
                always {
                    jacoco(execPattern: "**/jacoco.exec", exclusionPattern: "de/shadowhunt/maven/plugins/properties/HelpMojo.class", inclusionPattern: "de/shadowhunt/maven/plugins/properties/**/*")
